...
class NaiveBayesClassifier:
    ...
    mode : str
        ...
        only considers words that occur in the training data many times as only
        occuring once.
    delta : float
    ...
    ... [1, 0]
    """
    def __init__(self, mode: str = "count", delta: float = 1):
        self.mode = mode
        self.delta = delta
        self.p_neg = None
        self.p_pos = None
    ...
    def predict(self, x: list) -> list:
        ...
        y_hat = []
        for x_i in x:
            neg = self.p_neg * math.prod(self.p_w_neg[word] for word in x_i.split() if word in self.p_w_neg)
            pos = self.p_neg * math.prod(self.p_w_pos[word] for word in x_i.split() if word in self.p_w_pos)
